...
export function useSetState<S = object>(
  ...
  S,
  (nextSate: Partial<S> | ((prevState: S) => Partial<S>), callback?: (state: S) => any) => void
] {
  // @ts-ignore
  invariant(toRawType(initSate) === 'Object', 'initState should be a object')
  const [state, dispatch] = useReducer<
    (state: S, action: { type: 'update'; nextState: Partial<S> }) => S
  >(logger(setStateReducer), initSate)
  const updated = useRef<((state: S) => any) | undefined>(initCallback)
  /** make sure update */
  const setState = useCallback(
    (nextState: Partial<S> | ((prevState: S) => Partial<S>), callback?: (state: S) => any) => {
      // @ts-ignore
      invariant(
